    ...
    switch (name) {
      case 'figma_get_node': {
        ...
        const data = await figmaFetch(
          `/files/${fileKey}/nodes?ids=${normalizedNodeId}&depth=${depth}`
        );
      ...
      case 'figma_get_image': {
        ...
        const data = await figmaFetch(
          `/images/${fileKey}?ids=${normalizedNodeId}&scale=${scale}&format=${format}`
        );
      ...
      case 'figma_search': {
        ...
        const matches: { id: string; name: string; type: string; path: string[] }[] = [];
        
        function searchNodes(node: any, path: string[] = []) {
